...
pub fn start(config: &Config) {
    ...
    Command::new("curl").arg("-L").arg(&config.etcd_url).arg("-o").arg(format!("etcd/{}.tar.gz", folder_name)).status().expect("Error happened when trying to download `etcd`");
    if PathBuf::from(format!("/rustadm/etcd/{}.tar.gz", folder_name)).is_file() {
        tracing::info!("etcd downloaded");

        tracing::info!("untaring downloaded file");
        Command::new("tar").arg("-zxf").arg(format!("etcd/{}.tar.gz", folder_name)).arg("--directory").arg("etcd").status().expect("Error happened when trying to untar `etcd` executable");
        ...
        Command::new("cp").arg(format!("etcd/{}/etcd", folder_name)).arg(format!("etcd/{}/etcdctl", folder_name)).arg("/opt/etcd/bin").status().expect("Error happened when trying to copy `etcd` executable to `/opt/etcd/bin`");
